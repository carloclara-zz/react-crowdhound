...
class Contentservice {
  ...
  constructor(options) {
    if (!options) {
      console.error(
        `Contentservice was passed null options, so will be disabled.`
      )
      this.disabled = true
      return
    }
    ...
    //    fa (font-awsome 4)
    //    fas (font-awsome 5)
    this.defaultIconPack = options.defaultIconPack
      ? options.defaultIconPack
      : 'fa'
    this.icons = (pack) => {
      return this.defaultIconPack === pack
    }
  ...
  setPropertyInElement({ vm, element, name, value }) {
    console.log(`$crowdhound.setPropertyInElement`, vm, element, name, value)
    ...
    return new Promise((resolve, reject) => {
      ...
      if (arguments.length === 3) {
        ...
        if (typeof anchor != 'string') {
          console.log(
            'CrowdHound.select: anchor must be a string: ' +
              anchor +
              ', ' +
              typeof anchor
          )
        ...
        if (anchor.charAt(0) != '$') {
          console.log(
            "CrowdHound.select: anchor must start with '$': " + anchor
          )
          return reject(
            new Error("invalid anchor parameter - must start with '$'")
          )
        }
        if (typeof anchorType != 'string') {
          console.log(
            'CrowdHound.select: anchorType must be a string: ' +
              anchorType +
              ', ' +
              typeof anchorType
          )
      ...
      axios({
        ...
        .then((response) => {
          ...
          let userdata = null
          if (
            reply instanceof Array &&
            reply.length > 0 &&
            reply[0].__currentUser
          ) {
            userdata = reply[0]
   ...
   */
  update(vm, element) {
    console.log(`Crowdhound.js:update()`, element)
   ...
   */
  create(vm, element) {
    console.log(`Crowdhound.js:create()`, element)
    console.log(`element.description.length=`, element.description.length)
    return new Promise((resolve, reject) => {
      if (this.options.debug) {
        console.log('select()')
      ...
      const url = `${this.endpoint()}/element`
      axios({
        method: 'post',
        url,
        headers: {
